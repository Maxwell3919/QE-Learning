...
- SOC/noncollinear 会改变 Hamiltonian、spinor 数据链、对称性和赝势要求；SOC bands/DOS/topology 不能继承 scalar-relativistic 数据链。
- topology、magnetic anisotropy 或 spin texture 判断需要额外 symmetry、Wannier 或 invariant 证据，不能只靠 `lspinorb` 与 bands 图。
- 结论边界见 [physics-judgement/soc-and-symmetry-boundary.md](../../physics-judgement/soc-and-symmetry-boundary.md)。
