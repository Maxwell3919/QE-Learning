...
8. 结束状态：区分 `JOB DONE`、SCF/ionic convergence、walltime kill、scheduler error 和 library/runtime error。
...
- `BLOCK`：`prefix/outdir` 与记录不一致、scratch 丢失、restart 来源不明、赝势文件对不上、output 与 command 对不上，或 scheduler/runtime 错误导致结果不完整。
