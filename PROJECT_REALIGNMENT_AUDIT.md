...
本轮审计先读仓库现状，再按信息架构、workflow、最低理论、物理判断、来源边界和 standards 六个工作流检查。审计结论先记录缺口和优先级，后续修改应小批量推进，避免把项目写成自动化 runtime、工具接口规范或具体材料案例库。
...
| `physics-judgement/CONCLUSION_REFERENCE_BOOK.md` | 开头出现“agent 分工”语境 | 改成“页面维护和审阅分工” | P0 |
...
未发现需要引入 agent runtime、MCP、workflow engine 或 VibeDFT 执行系统的内容。后续维护应继续避免这些方向进入正文。
...
每个阶段都必须运行：Markdown 本地链接检查、禁用词/本地路径/固定材料案例扫描、README 与目录索引一致性检查、`git diff --check`。
